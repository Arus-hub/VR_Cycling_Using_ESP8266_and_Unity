...
public class UdpCL : MonoBehaviour
{		
	...
	void Update ()
	{
		...
		if (received != ""){
			...
			{
				HandleVertical(received);
			}
			else
			{
				HandleHorizontal(received);
			}
	...
	void HandleVertical(string vert)
	{
		...
		{
			outsideControls.Vertical = 0.5f/1.112f;
		}
		else if (vert.Contains("_NO"))
        {
			outsideControls.Vertical = 0.0001f;
        }
	}
